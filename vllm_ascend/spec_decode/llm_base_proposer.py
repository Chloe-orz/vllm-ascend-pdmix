...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def load_model(self, model: nn.Module) -> None:
        ...
        if allows_cloud_only_attention:
            # DeepSeek-V4 does not construct its cloud-owned MTP decoder block
            # on the edge, so no local draft attention layer is registered.
            # The edge still needs a block size while preparing the metadata
            # sent to the cloud; it uses the target model's configured size.
